...
export class Walker {
  constructor(ground, obstacles, {spawn=[-1.4,-12],eyeHeight=1.75,yaw=0,pitch=.07}={}) { this.ground=ground;this.bounds=ground.bounds??{xmin:-54,xmax:54,ymin:-24,ymax:130};this.obstacles=obstacles;this.spawn=spawn;this.eyeHeight=eyeHeight;this.startYaw=yaw;this.startPitch=pitch;this.reset(); }
  ...
  step(dt, forward, strafe, fast=false) {
    ...
    for(let i=0;i<steps;i++) {
      let x=Math.max(this.bounds.xmin,Math.min(this.bounds.xmax,this.x+dx/steps)),y=Math.max(this.bounds.ymin,Math.min(this.bounds.ymax,this.y+dy/steps));
      ...
      x=Math.max(this.bounds.xmin,Math.min(this.bounds.xmax,x));y=Math.max(this.bounds.ymin,Math.min(this.bounds.ymax,y));
