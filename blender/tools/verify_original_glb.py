...
path = Path(sys.argv[1]).resolve() if len(sys.argv)>1 else root / 'exports/everest-original-full.glb'
...
assert len(asset['scenes']) == 1 and asset.get('scene',0) == 0
assert not any(node.get('name') == 'Cube' for node in asset['nodes'])
...
assert len(mesh_nodes) == expected + int(report.get('sky_mesh',True)), (len(mesh_nodes), expected)
